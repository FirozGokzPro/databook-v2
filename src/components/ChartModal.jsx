...
export default function ChartModal({ chart, onClose }) {
  ...
  const renderChart = () => {
    if (chart.id === 'bar') {
      return (
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chart.data} barGap={4} margin={{ top: 16, right: 24, bottom: 8, left: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#1e1e2a" vertical={false} />
            <XAxis dataKey="month" tick={{ fill: '#7777aa', fontSize: 13 }} axisLine={false} tickLine={false} />
            <YAxis tick={{ fill: '#7777aa', fontSize: 13 }} axisLine={false} tickLine={false} />
            <Tooltip content={<CustomTooltip />} cursor={{ fill: '#ffffff06' }} />
            <Legend wrapperStyle={{ paddingTop: 16, fontSize: 13, color: '#7777aa' }} />
            <Bar dataKey="revenue" fill="#8b5cf6" radius={[4, 4, 0, 0]} name="Revenue" />
            <Bar dataKey="target" fill="#2a2a3a" radius={[4, 4, 0, 0]} name="Target" />
          </BarChart>
        </ResponsiveContainer>
      )
    }
    if (chart.id === 'pie') {
      ...
      const sliceColor = activeSlice ? PIE_COLORS[chart.data.findIndex(d => d.name === activeSlice.name)] : '#8b5cf6'
            ...
            <ResponsiveContainer width="100%" height="75%">
              <PieChart>
                <Pie
                  ...
                  paddingAngle={4}
                  dataKey="value"
                  onClick={(d) => setActiveSlice(prev => prev?.name === d.name ? null : d)}
                  cursor="pointer"
                  isAnimationActive
                ...
                <Tooltip formatter={(v) => `${v}%`} contentStyle={{ background: '#1a1a24', border: '1px solid #2e2e3e', borderRadius: 8, fontSize: 13 }} />
              </PieChart>
            </ResponsiveContainer>
            <div className="pie-modal-legend">
              {chart.data.map((d, i) => (
                <div
                  ...
                  onClick={() => setActiveSlice(prev => prev?.name === d.name ? null : d)}
                >
                  <span className="pie-modal-dot" style={{ background: PIE_COLORS[i] }} />
                  <span className="pie-modal-name">{d.name}</span>
                  <span className="pie-modal-pct">{d.value}%</span>
                </div>
              ))}
            </div>
            {!activeSlice && <p className="pie-click-hint">Click a slice to drill down</p>}
          </div>

          {/* Drill-down bar chart — slides in from right */}
          <div className={`pie-modal-right ${activeSlice ? 'drill-visible' : ''}`}>
            {drillData && (
              <>
                <div className="drill-header">
                  <span className="drill-dot" style={{ background: sliceColor }} />
                  <span className="drill-title">{activeSlice.name} — Breakdown</span>
                  <button className="drill-close" onClick={() => setActiveSlice(null)}>
                ...
                <ResponsiveContainer width="100%" height="80%">
                  <BarChart data={drillData} layout="vertical" margin={{ top: 8, right: 24, bottom: 8, left: 8 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#1e1e2a" horizontal={false} />
                    <XAxis type="number" tick={{ fill: '#7777aa', fontSize: 12 }} axisLine={false} tickLine={false} />
                    <YAxis type="category" dataKey="label" tick={{ fill: '#aaaacc', fontSize: 13 }} axisLine={false} tickLine={false} width={72} />
                    <Tooltip
                      formatter={(v) => [`${v}%`, 'Share']}
                      contentStyle={{ background: '#1a1a24', border: '1px solid #2e2e3e', borderRadius: 8, fontSize: 13 }}
                      cursor={{ fill: '#ffffff06' }}
                    />
                    <Bar dataKey="value" fill={sliceColor} radius={[0, 4, 4, 0]} name="Share" isAnimationActive animationDuration={600} animationEasing="ease-out" />
                  </BarChart>
                </ResponsiveContainer>
      ...
      return (
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chart.data} margin={{ top: 16, right: 24, bottom: 8, left: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#1e1e2a" vertical={false} />
            <XAxis dataKey="week" tick={{ fill: '#7777aa', fontSize: 13 }} axisLine={false} tickLine={false} />
            <YAxis tick={{ fill: '#7777aa', fontSize: 13 }} axisLine={false} tickLine={false} />
            <Tooltip contentStyle={{ background: '#1a1a24', border: '1px solid #2e2e3e', borderRadius: 8, fontSize: 13 }} />
            <Legend wrapperStyle={{ paddingTop: 16, fontSize: 13, color: '#7777aa' }} />
            <Line type="monotone" dataKey="users" stroke="#8b5cf6" strokeWidth={2.5} dot={{ fill: '#8b5cf6', r: 4 }} name="Users" />
            <Line type="monotone" dataKey="sessions" stroke="#06b6d4" strokeWidth={2.5} dot={{ fill: '#06b6d4', r: 4 }} name="Sessions" />
          </LineChart>
        </ResponsiveContainer>
      )
    }
        ...
        <div className="modal-header">
          <div className="modal-title-row">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
              ...
              <path d="M3 9h18M9 21V9" stroke="#8b5cf6" strokeWidth="2" strokeLinecap="round"/>
            </svg>
          ...
          <div className="modal-chat-log">
            ...
            {loading && (
              <div className="modal-msg modal-msg-ai">
                <span className="modal-ai-label">AI</span>
                <span className="modal-thinking"><span /><span /><span /></span>
              </div>
            )}
        ...
        <div className="modal-input-area">
          <div className="modal-input-wrapper">
            ...
            <input
              ...
              onKeyDown={handleKey}
            />
            <button className={`modal-send ${input.trim() ? 'active' : ''}`} onClick={send} disabled={!input.trim() || loading}>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none">
